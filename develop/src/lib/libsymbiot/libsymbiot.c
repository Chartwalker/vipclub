...
FILE *libsymbiot_file_open(Main_Init_t * Main_Init_p, const char *file_fn,
			   const char *file_fm)
{
	// create file_fp
	static FILE *temp_fp;
	...
	if ((temp_fp = fopen(file_fn, file_fm)) == NULL) {
		if (Main_Init_p->err_fp) {
			fprintf(Main_Init_p->err_fp,
				"\n%sunable to fopen(%s,%s) - Error!\n", Dbg_s,
				file_fn, file_fm);
		}
	} else {
		if (Main_Init_p->log_fp) {
			fprintf(Main_Init_p->log_fp,
...
Libsymbiot_Conf_t *libsymbiot_intelhex(Libsymbiot_Conf_t * Parent_p)
{
	...
	if (malloc_p == NULL) {
		...
		if (Parent_p) {
			...
			fprintf(Parent_p->log_fp,
				"iot_ihex: creating %%p(Buf_p)=%p and linking to %%p(Parent_p)=%p\n",
				Parent_p->Buffer_p, Parent_p);
			fprintf(Parent_p->log_fp,
				"iot_ihex: at %%p(Buf_p)=%p offset=%lu with start=%p end=%p size=%lu\n",
				Buffer_p->buf_p,
				(char *)Buffer_p->start_p -
				((char *)Parent_p->Buffer_p), Buffer_p->start_p,
				Buffer_p->end_p,
				Buffer_p->end_p - Buffer_p->start_p);
		} else {
			fprintf(stderr,
				"iot_ihex: unable to linking %%p(Buf_p)=%p to Parent_p=%p\n",
				Buffer_p, Parent_p);
		}
	}
	// for(i=0;j<8;j++){[
...
Libsymbiot_Conf_t *libsymbiot_conf_new(Libsymbiot_Conf_t * Parent_p)
{
	...
	if (mem_p == NULL) {
		fprintf(stderr, "iot_conf_new: unable to get memory at %p",
			mem_p);
		return NULL;
	} else {
		// out_p=mem_p;
...
Libsymbiot_Conf_t *libsymbiot_conf_free(Libsymbiot_Conf_t * Child_p)
{
	...
	if (Conf_p) {
		if (Conf_p->log_fp) {
			fprintf(Conf_p->log_fp,
				"iot_conf_free: %%p(Conf_p)=%p  sz=%lu\n",
				Conf_p, sizeof(*Conf_p));
		} else {
			fprintf(stderr,
				"iot_conf_free: unable to get log_fp at %p\n",
				Conf_p);
		}
		// step uo      
		...
		if (Parent_p) {
			fprintf(Parent_p->log_fp,
				"iot_conf_free: %%p(Parent Conf_p)=%p  sz=%lu\n",
				Parent_p, sizeof(*Parent_p));
		} else {
			fprintf(stderr,
				"iot_conf_free: error: %%p(Parent Conf_p)=%p (invalid)\n",
				Parent_p);
		}
	} else {
		fprintf(stderr,
